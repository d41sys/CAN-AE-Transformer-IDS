...
class Model:
    def __init__(self, model='AAE', data_dir='./Data/', unknown_attack=None, input_dim=29 * 29, z_dim=10,
                 batch_size=100, n_epochs=100, supervised_lr=0.0001, reconstruction_lr=0.0001,
    ...
    def construct_data_flow(self):
        ...
        train_label_paths = ['{}/{}/train_label'.format(self.data_dir, l) for l in self.labels if
                             l != self.unknown_attack]
        ...
        train_unlabel = data_from_tfrecord(train_unlabel_paths, self.batch_size - self.batch_size_unknown,
                                           self.n_epochs)
    ...
    def build(self):
        ...
        self.manual_decoder_input = tf.placeholder(dtype=tf.float32, shape=[1, self.z_dim + self.n_labels],
                                                   name='Decoder_input')
        ...
        self.discriminator_g_optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate,
                                                                beta1=self.beta1, beta2=self.beta2).minimize(
            self.dc_g_loss, var_list=dc_g_var)
        self.discriminator_c_optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate,
                                                                beta1=self.beta1, beta2=self.beta2).minimize(
            self.dc_c_loss, var_list=dc_c_var)
        # Generator loss
        self.generator_loss = -tf.reduce_mean(d_g_fake) - tf.reduce_mean(d_c_fake)

        en_var = [var for var in all_variables if 'e_' in var.name]
        self.generator_optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate, beta1=self.beta1,
                                                          beta2=self.beta2).minimize(self.generator_loss,
                                                                                     var_list=en_var)
        ...
        self.supervised_encoder_loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(labels=self.y_input, logits=self.encoder_output_label_))
        self.supervised_encoder_optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate,
                                                                   beta1=self.beta1_sup).minimize(
            self.supervised_encoder_loss, var_list=en_var)
    ...
    def get_val_acc(self, val_size, batch_size, tfdata, sess):
        ...
        for j in tqdm.tqdm(range(num_batches)):
            ...
            batch_pred = sess.run(self.output_label,
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            tensorboard_path, saved_model_path, log_path = form_results(self.model_name, self.results_path, self.z_dim,
                                                                        self.supervised_lr, self.batch_size,
                                                                        self.n_epochs, self.beta1)
            sess.run(init)
            ...
            for epoch in range(self.n_epochs):
                ...
                for b in tqdm.tqdm(range(1, n_batches + 1)):
                    ...
                    sess.run(self.autoencoder_optimizer, feed_dict={self.x_input: batch_x_ul, self.x_target: batch_x_ul,
                                                                    self.learning_rate: self.reconstruction_lr,
                                                                    self.keep_prob: 0.75})
                    ...
                    sess.run(self.generator_optimizer, feed_dict={self.x_input: batch_x_ul, self.x_target: batch_x_ul,
                                                                  self.learning_rate: self.regularization_lr,
                                                                  self.keep_prob: 0.75})

                    sess.run(self.supervised_encoder_optimizer,
                             feed_dict={self.x_input_l: batch_x_l, self.y_input: batch_y_l,
                                        self.learning_rate: self.supervised_lr, self.keep_prob: 0.75})
                    if b % 10 == 0:
                        a_loss, d_g_loss, d_c_loss, g_loss, s_loss, summary = sess.run(
                            [self.autoencoder_loss, self.dc_g_loss, self.dc_c_loss, self.generator_loss,
                             self.supervised_encoder_loss,
                             self.summary_op],
                            feed_dict={self.x_input: batch_x_ul, self.x_target: batch_x_ul,
                                       self.real_distribution: z_real_dist, self.y_input: batch_y_l,
                                       self.x_input_l: batch_x_l,
                ...
                if (epoch + 1) % 2 == 0:
                    ...
                    acc_known, precision_known, recall_known, f1_known = self.get_val_acc(self.validation_size,
    ...
    def test(self, results_path, unknown_test):
        ...
        with tf.Session() as sess:
            ...
            test = data_from_tfrecord(tf_filepath=[p + 'test' for p in data_path], batch_size=self.batch_size,
...
if __name__ == '__main__':
    ...
    model = Model(model=args.model, data_dir=args.data_dir, unknown_attack=args.unknown_attack,
